...
int p4(PCAR pCar)
{
	...
	{		
		...
		// }
		// if(pCar->hasMileage >= 0 && pCar->hasMileage < 100)
		// {
			// pCar->hasMileage++;
		// }
		...
		if(mouse_press(370, 104, 443, 137) == 1 && pCar->runState != 1 && pCar->electricityLeft > 0)			//行驶状态
		{
			pCar->k = 10;
			pCar->runState = 1;			
			pCar->start = clock();
		...
		if(mouse_press(451, 104, 522, 137) == 1 && pCar->runState != 2 && pCar->electricityLeft > 0)		//制动状态
		{
			pCar->k = 20;
			pCar->runState = 2;			
			clrmous(mouseX,mouseY);
